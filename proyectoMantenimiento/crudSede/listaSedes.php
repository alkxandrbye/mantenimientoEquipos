                ...
                <ul class="sidebar-nav">
                    ...
                    <li class="sidebar-item">
                        ...
                        <ul id="sede" class="sidebar-dropdown list-unstyled collapse" data-bs-parent="#sidebar">
                            <li class="sidebar-item">
                                <a href="registrarSede.html" class="sidebar-link">Registrar</a>
                            </li>
                            <li class="sidebar-item">
                                <a href="listaSedes.php" class="sidebar-link">Consultar</a>
                            </li>
                            </li>
                        </ul>
                    </li>
                        ...
                        <tbody>
                            <?php 
                                while ($row = $result->fetch_array()) {
                                    echo '<tr><td>' . $row["idSede"] . '</td>';
                                    echo '<td>' . $row["nombreSede"] .'</td>';
                                    echo '<td> <button type="button" class="btn btn-primary btn-sm" onclick="abrirModalActualizar('.$row["idSede"].', \''.$row["nombreSede"].'\')">Editar</button></td>
                                    <td> <a href="#" onclick="confirmarEliminar('.$row["idSede"].')" class="btn btn-danger btn-sm">Eliminar</td>
                                    </tr>';
                                }
                                $result->free_result();
                                $conexion->close();
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalActualizarLabel">Actualizar sede</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formActualizar" method="POST" action="updateSede.php">
                        <div class="mb-3">
                            <label for="nombreSede" class="form-label">Nombre</label>
                            <input type="text" class="form-control" id="nombreSede" name="nombreSede">
                        </div>
                        <div class="mb-3">
                            <label for="idSede" class="form-label">Id</label>
                            <input type="text" class="form-control" id="idSede" name="idSede" readonly>
                        </div>
                        <button onclick="actualizar(document.getElementById('idSede').value)" class="btn btn-primary">Actualizar</button>
                    </form>
                </div>
            </div>
        </div>
